...
int gameMode = 0, gameDifficulty = 0, restart, faults, endGame;
int process;
int flag_w = 1;
int num = 1;
int sleepTime = 0;
...
void select1()
{
	...
	while (post)
	{
		...
		case WM_MOUSEMOVE:
			currentPos.X = x;
			currentPos.Y = y;
			//printf("%u-%u", currentPos.X, currentPos.Y);
			break;
		case WM_LBUTTONUP:
...
void start1()
{
	...
	settextcolor(BLACK);	// 设置文字为黑色
	setbkmode(1);			// 设置文字背景透明
	cleardevice();

	LOGFONT f;
	gettextstyle(&f);		// 获取当前字体设置
	f.lfHeight = 100;		// 设置字体高度为100
