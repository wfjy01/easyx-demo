...
void Game::Draw()
{
	static Button btn_menu(WIDTH - 120, 10, 100, 50, "选项", 28);
...
{
	if (cosmo->on->mode  & F1) {
		if (!(fabs((cosmo->on->y - HEIGHT / 2) + 0.5*(WIDTH / 2 - cosmo->on->x)) <= cosmo->on->LENOFSIDE + 1 &&
			fabs((cosmo->on->y - HEIGHT / 2) - 0.5*(WIDTH / 2 - cosmo->on->x)) <= cosmo->on->LENOFSIDE + 1))
			return true;
	}
	else if (cosmo->on->mode  & F2) {
		if (!(fabs((cosmo->on->y - HEIGHT / 2) + 0.5*(WIDTH / 2 - cosmo->on->x)) <= cosmo->on->LENOFSIDE + 1 &&
			fabs(WIDTH / 2 - cosmo->on->x) <= cosmo->on->LENOFSIDE + 1))
			return true;
	}
	else if (cosmo->on->mode  & F3) {
		if (!(fabs((cosmo->on->y - HEIGHT / 2) - 0.5*(WIDTH / 2 - cosmo->on->x)) <= cosmo->on->LENOFSIDE + 1 &&
