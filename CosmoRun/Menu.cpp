...
{
	// 获取显示菜单前的背景
	getimage(&background, 0, 0, WIDTH, HEIGHT);
	setfillcolor(menu_color);
	setlinecolor(0xcccccc);
	for (int i = 1; i <= MENU_SPEED; i++) {
		solidroundrect((WIDTH - wid) / 2, HEIGHT / 2 - len / 2 * i / MENU_SPEED, (WIDTH + wid) / 2, HEIGHT / 2 + len / 2 * i / MENU_SPEED, 60, 20);
		FlushBatchDraw();
		sleep();
	}
}
...
void Menu::Hide()
{
	...
	for (int i = MENU_SPEED; i >= 0; i--) {
		...
		fillroundrect((WIDTH - wid) / 2, HEIGHT / 2 - len / 2 * i / MENU_SPEED, (WIDTH + wid) / 2, HEIGHT / 2 + len / 2 * i / MENU_SPEED, 60, 20);
		FlushBatchDraw();
		sleep();
	}
	putimage(0, 0, &background);
