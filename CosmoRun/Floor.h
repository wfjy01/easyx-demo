...
private:
	...
	float CosmoOnColor;
	bool create_bug; // 板块创建出错时为true
	bool FitRule(); // 初步，检测板块类型是否符合规则
	bool JudgeCoincidence(); // 进一步， 检查板块是否重合
	void GetLocation(); //根据板块类型得到坐标
	void draw_f(double x1, double x2, double x3, double x4, double y1, double y2, double y3, double y4); // 画四边形;
};
